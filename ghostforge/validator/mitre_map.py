...
DEFAULT_DAG: Dict[str, StageNode] = {
    "Reconnaissance": StageNode(name="Reconnaissance", requires=[], evidence=["port_scan", "syn_flood"]),
    "InitialAccess": StageNode(name="InitialAccess", requires=["Reconnaissance"], evidence=["brute_force", "exploit"]),
    "Discovery": StageNode(name="Discovery", requires=["InitialAccess"], evidence=["dns_query", "port_scan"]),
    "LateralMovement": StageNode(name="LateralMovement", requires=["Discovery"], evidence=["smb_445", "winrm_5985", "rpd"]),
    "CommandAndControl": StageNode(name="CommandAndControl", requires=["LateralMovement"], evidence=["dns_tunnel", "http_beacon"]),
    "Exfiltration": StageNode(name="Exfiltration", requires=["CommandAndControl"], evidence=["large_outbound", "dns_tunnel"]),
}
